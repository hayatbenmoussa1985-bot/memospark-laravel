<x-user-layout :title="'Edit: ' . $deck->title">

    <div class="mb-4">
        <a href="{{ route('user.decks.show', $deck) }}" class="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
    ...
    <div class="max-w-lg">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-6">Edit Deck</h2>
            <form method="POST" action="{{ route('user.decks.update', $deck) }}">
                @csrf @method('PUT')
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" class="w-full text-sm border-gray-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('category_id', $deck->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->icon }} {{ $cat->slug }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="mt-6 flex items-center justify-between">
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('user.decks.show', $deck) }}" class="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</a>
                    </div>
                    <form method="POST" action="{{ route('user.decks.destroy', $deck) }}" onsubmit="return confirm('Delete this deck and all its cards?')">
                        @csrf @method('DELETE')
                        <button type="submit" class="px-4 py-2 text-sm text-red-600 border border-red-200 rounded-lg hover:bg-red-50">Delete</button>
                    </form>
                </div>
            </form>
        </div>
